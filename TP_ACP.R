...
#_______________________________________
  ...
  getwd()
  setwd("C:/Users/dell/Downloads/BASE_ACP")
  getwd()
...
#_____________________________________
  ...
  #-------------------------------
  # La moyenne
  var_mean <- Data_PCA_wide %>% summarise_all(mean)
  var_mean <- as.data.frame(t(var_mean))
  var_mean <- var_mean %>% rename("Mean"="V1")
  View(var_mean)
...
#________________________________________
  ...
  eig.value$dimension = seq.int(from=1, to=24, by=1)
  View(eig.value)
...
#__________________________________
  ...
  # Mise en element supplementaire d'un individu
  PCA_result_supp1 <- PCA(Data_PCA_supp[,1:27],scale.unit = T, ncp = 10,ind.sup=1, graph = FALSE) 
  ...
  PCA_result_supp2 <- PCA(Data_PCA_supp,scale.unit = T, ncp = 10, quali.sup=28 ,graph = FALSE) 
  ...
  PCA_result_supp3 <- PCA(Data_PCA_supp[,1:27],scale.unit = T, ncp = 10,quanti.sup=27,graph = FALSE) 
